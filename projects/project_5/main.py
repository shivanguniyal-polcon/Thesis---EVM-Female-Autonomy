...
# ==========================================
# PHASE 1: DATA INGESTION & SPATIAL CROSSWALK (FIXED)
# ==========================================
...
#FIX: Cap definitionally impossible values at 100%
...
# ✅ FIX: Turnout aggregation NO LONGER computes or carries EVM exposure
...
# ✅ FIX: Compute EVM exposure STATICALLY from 1999 data only
def compute_evm_exposure(file_name_1999):
    ...
    if missing_treated:
        print(f"⚠️ Treated PCs that FAILED to merge: {missing_treated}")
    else:
...
# 5. Build Turnout Panel (Exposure removed)
panel = pd.concat([
...
# 7. Pivot for Cross-Sectional ANCOVA/DDD
...
# ✅ FIX: Rename 1998 and require it for the panel balance
...
# 8. Merge static 1999 exposure into the wide panel
...
print(f"\nFinal Clean Sample: {len(wide)} districts.")
print(f"Treated Districts (EVM Exposure > 0): {(wide['EVM_Exposure_Cont'] > 0).sum()}")


# ==========================================
# SPECIFICATION HORSE RACE FUNCTION (Unchanged)
# ==========================================
...
# ==========================================
# PHASE 2: MAIN CAUSAL MODEL (Mean-Centered)
# ==========================================
...
print("\n--- FINAL MODEL COEFFICIENTS (Mean-Centered) ---")
res_df = pd.DataFrame({
...
# ==========================================
# MARGINAL EFFECTS OF EVM ACROSS AGENCY DISTRIBUTION
# ==========================================
print("\n--- MARGINAL EFFECTS OF EVM ACROSS AGENCY DISTRIBUTION ---")
...
print("\n--- GENERATING MAIN MODEL FOREST PLOT ---")
...
ax.set_yticks(y_pos)
ax.set_yticklabels(['EVM Exposure (Main)', 'Economic Agency (Main)', 'EVM × Agency (Interaction)'], fontsize=11)
...
print("\n--- GENERATING COVARIATE BALANCE LOVE PLOT ---")
...
print("\n--- GENERATING FWL PARTIAL REGRESSION PLOT ---")
fig, ax = plt.subplots(figsize=(8, 6))
...
ax.set_xlabel('EVM × Agency (Residualized)', fontsize=12)
...
print("\n--- GENERATING FWL PARTIAL RESIDUAL PLOTS FOR ALL COVARIATES ---")
# Partial Residuals (Component + Residual) visualize the FWL theorem by showing 
# the isolated relationship between Y and X_j after partialling out all other covariates.

from statsmodels.nonparametric.smoothers_lowess import lowess

# List all continuous covariates in your final model
continuous_covariates = [
    'EVM_Exposure_Cont', 'Agency_Centered', 'Urban_Pct_ihs', 
    'Turnout_96', 'Lit_Pct', 'SC_Pct', 'ST_Pct_ihs'
]

# Setup Grid (3 columns, enough rows to fit all 7 variables)
n_cols = 3
...
for i, cov in enumerate(continuous_covariates):
    ...
    # 4. Add LOWESS smoother to check for non-linearity (FWL diagnostic)
...
if not loo_df.empty:
    ...
    print("⚠️ LOO returned no results. Check if states have enough variation.")
...
if not loo_df.empty:
    ...
    ax.set_xlabel('Interaction Coefficient (EVM × Agency)', fontsize=12)
...
print("\n--- GENERATING SUB-SAMPLE HETEROGENEITY PLOT ---")
...
ax.set_xlabel('Interaction Effect (EVM × Agency)', fontsize=12)
...
# VIF & CLUSTERED SEs
# ==========================================
print("\n--- VIF FOR CONTINUOUS REGRESSORS (Excluding State FEs) ---")
...
print("\n--- RE-ESTIMATING WITH STATE-CLUSTERED STANDARD ERRORS ---")
...
print("\n--- GENERATING VIF DIAGNOSTICS PLOT ---")
...
# ==========================================
# COOK'S DISTANCE & MARGINAL EFFECTS PLOTS
# ==========================================
...
# ==========================================
# IPW & ROBUSTNESS CHECKS
# ==========================================
# ==========================================
# ROBUSTNESS: IPW (Consolidated & Scoping Fixed)
# ==========================================
print("\n--- ROBUSTNESS: PROPENSITY SCORE WEIGHTING (IPW) ---")

wide['EVM_Any'] = (wide['EVM_Exposure_Cont'] > 0).astype(int)
wide['IPW_Interaction'] = wide['EVM_Any'] * wide['Agency_Centered']

# ✅ FIX: Define OUTSIDE any try block so both methods can access it
ipw_formula = (
...
try:
    ...
    print(f"⚠️ Statsmodels IPW failed (likely separation): {e}")
...
try:
    ...
    print(f"⚠️ Sklearn IPW failed: {e}")

print("\n--- ROBUSTNESS: EXCLUDING COOK'S DISTANCE OUTLIERS ---")
...
print("⚠️ High-Leverage Outliers Identified:")
print(outlier_info)
...
print("\n--- ROBUSTNESS: PROPENSITY SCORE WEIGHTING (IPW via sklearn) ---")
features = ['Lit_Pct', 'Urban_Pct_ihs', 'ST_Pct_ihs', 'SC_Pct', 'Turnout_96']
X_ps = wide[features].copy()
y_ps = wide['EVM_Any']
scaler = StandardScaler()
...
try:
    ...
    print(f"⚠️ WLS estimation failed: {e}")

print("\n--- GENERATING IPW OVERLAP PLOT ---")
...
print("\n--- ROBUSTNESS: IPW WITH TRIMMED WEIGHTS ---")
...
print("\n--- DIAGNOSTIC: TREATMENT SPARSITY & OVERLAP ---")
...
# ==========================================
# 1998 PRE-TREND TEST (Parallel Trends Validation)
# ==========================================
print("\n" + "="*70)
...
if 'Turnout_98' not in wide.columns:
    print("⚠️ Turnout_98 not found. Ensure the 1998 rename patch was applied to the pivot.")
else:
    wide['Delta_9698'] = wide['Turnout_98'] - wide['Turnout_96']
    
    # EVM_Post_Agency is already defined as (EVM_Exposure_Cont * Agency_Centered)
    # In this placebo regression, it tests if the heterogeneous trend existed pre-1999
    pretrend_formula = (
    ...
    try:
        ...
        if pretrend_mod.pvalues['EVM_Post_Agency'] > 0.10:
            ...
            print("⚠️ WARNING: Significant pre-trend detected.")
            print("   Differential trends by agency level existed before the 1999 EVM rollout.")
    except Exception as e:
        print(f"⚠️ Pre-trend test failed: {e}")


# Verify pre-trend sample composition
...
try:
    ...
    ax.set_xlabel('Coefficient of EVM × Agency Interaction', fontsize=12)
...
# ==========================================
# MECHANISM TEST: ECONOMIC AGENCY vs. DEEP-ROOTED PATRIARCHY (Z-SCORED)
# ==========================================
print("\n" + "="*70)
print("  MECHANISM HORSE RACE: ECONOMIC AGENCY vs. PATRIARCHY (Z-SCORED)")
print("="*70)

# 1. Extract Child Sex Ratio (0-6) from 1991 Census
...
if fem_col in census.columns and male_col in census.columns:
    ...
    # 2. CRITICAL THESIS STEP: Z-Score Standardization
    # We divide by the standard deviation so both mechanisms are measured in "Standard Deviations".
    # This makes their coefficients directly comparable in magnitude.
    ...
    # 3. Run the "Horse Race" Regression
    horse_race_formula = (
    ...
    print("\n--- Z-SCORED HORSE RACE RESULTS (Dep Var: Turnout_99) ---")
    ...
    print(f"Economic Agency (1 SD increase):   Beta = {beta_eco:.4f}  |  p-value = {p_eco:.4f} {'***' if p_eco<0.01 else '**' if p_eco<0.05 else '*' if p_eco<0.10 else ''}")
    print(f"Cultural Patriarchy (1 SD increase): Beta = {beta_soc:.4f}  |  p-value = {p_soc:.4f} {'***' if p_soc<0.01 else '**' if p_soc<0.05 else '*' if p_soc<0.10 else ''}")
    ...
    if p_eco < 0.05 and p_soc > 0.10:
        ...
        print("⚠️ Neither mechanism is individually significant when controlling for the other.")
        
    corr = wide['Agency_Z'].corr(wide['Patriarchy_Z'])
    print(f"\nCorrelation between Z-scored Agency and Patriarchy: {corr:.3f}")

       # ==========================================
    # VISUALIZATION: MECHANISM HORSE RACE PLOT (FIXED)
    # ==========================================
    print("\n--- GENERATING MECHANISM HORSE RACE PLOT ---")
    ...
    # Calculate lower and upper error distances from the coefficient
    xerr_lower = [c - l for c, l in zip(coefs_race, lower_race)]
    xerr_upper = [u - c for c, u in zip(coefs_race, upper_race)]
    
    # Color code based on significance (Purple if p < 0.05, Grey if not)
    colors_race = ['#6A0DAD' if p < 0.05 else '#d3d3d3' for p in [p_eco, p_soc]]

    # ✅ FIX: Calculate buffer correctly using a list comprehension for abs()
    ...
    # ✅ Loop through each point to apply individual colors 
    for i in range(len(key_vars_race)):
        # Plot the errorbar and point
        ...
        # Add significance stars next to the point
        p_val = [p_eco, p_soc][i]
        star = '***' if p_val < 0.01 else '**' if p_val < 0.05 else '*' if p_val < 0.10 else 'n.s.'
        
        # Place text just outside the upper confidence bound
...
# 1. Export the Final Master Analytical Dataset
# This is the exact dataframe used for the final ANCOVA and Mechanism models.
...
# 2. Export Table 1: Comprehensive Summary Statistics
# Generates clean summary stats for all continuous variables used in the final models.
summary_vars = [
...
# Filter to only include variables that actually exist in the final dataframe
...
# 3. Export Correlation Matrix (Multicollinearity Defense)
# Exports the Pearson correlations between all continuous right-hand side variables.
...
# 4. Export Master Regression Table (Side-by-Side Coefficients)
# Combines the Final ANCOVA, Pre-Trend, and Mechanism models into one clean appendix table.
...
reg_tables = []
# Append all major models estimated in this final script
...
# 5. Export Robustness & Diagnostics Summary
# A single reference sheet for VIFs, Pre-trend checks, and Outlier counts.
robustness_rows = []
...
print("\n🎉 ALL THESIS APPENDIX CSVs SUCCESSFULLY GENERATED IN 'CSV_OUTPUTS' FOLDER!")
