...
# PCs with by-elections or data issues in 1998 that must be excluded for a clean pre-trend
...
def aggregate_year(file_name, year):
    ...
    df['pc_name_clean'] = df['pc_name_clean'].replace({
        'MUMBAI SOUTH CENTRA': 'MUMBAI SOUTH CENTRAL', 'PONDICHERRY': 'PUDUCHERRY'
    })
    
    # Drop contaminated 1998 PCs to ensure a clean placebo test
    if year == 1998:
...
print("\n[1] Aggregating 1996, 1998, and 1999 Elections to 1991 Districts...")
...
# Use 1999 EVM exposure as the definitive treatment assignment
panel['EVM_Exposure'] = panel['EVM_Exposure_1999']
print(f"✅ Balanced Panel Created: {len(panel)} districts across 3 election cycles.")

# ==========================================
# 2. COVARIATES & ECONOMIC AGENCY
# ==========================================
print("\n[2] Merging Demographics & Economic Agency...")
...
# Mean-Center Agency for Interaction Interpretation
agency_mean = panel['Agency_IHS'].mean()
panel['Agency_Centered'] = panel['Agency_IHS'] - agency_mean

# Calculate DiD Deltas
panel['Delta_PreTrend'] = panel['Turnout_1998'] - panel['Turnout_1996'] # Placebo
panel['Delta_DiD'] = panel['Turnout_1999'] - panel['Turnout_1996']      # Treatment

print(f"✅ Final Analytical Sample: {len(panel)} districts.")

# ==========================================
# 3. COMPREHENSIVE DIAGNOSTICS FUNCTION (FIXED)
# ==========================================
def print_full_diagnostics(model, model_name):
    print(f"\n{'='*20} DIAGNOSTICS: {model_name} {'='*20}")
    ...
    # ✅ FIX: Use statsmodels' jarque_bera which returns 4 values (stat, pval, skew, kurtosis)
    # scipy.stats.jarque_bera only returns 2 values, causing the unpacking error.
    ...
    if bp_pval < 0.05:
        print("  ⚠️ Heteroskedasticity detected. Robust SEs (HC1) applied.")
    print("-" * 60)
    ...
    if jb_pval < 0.05:
        print("  ⚠️ Residuals are non-normal. OLS unbiased, but rely on HC1 SEs.")
    print("=" * 60)

# ==========================================
# 4. ESTIMATION: PRE-TREND & DiD
# ==========================================
print("\n[3] ESTIMATING CAUSAL MODELS...")
controls = "Lit_Pct + SC_Pct + ST_Pct + Urban_Pct + C(state_clean)"
interaction = "EVM_Exposure * Agency_Centered"

# Model A: Placebo Pre-Trend (1996 -> 1998)
# If causal, the interaction term MUST be statistically insignificant here.
formula_pre = f"Delta_PreTrend ~ {interaction} + {controls}"
mod_pre = smf.ols(formula_pre, data=panel).fit(cov_type='HC1')

# Model B: Main Difference-in-Differences (1996 -> 1999)
formula_did = f"Delta_DiD ~ {interaction} + {controls}"
mod_did = smf.ols(formula_did, data=panel).fit(cov_type='HC1')

# Model C: ANCOVA (1999 Turnout controlling for 1996 Baseline)
# Often more statistically powerful than pure DiD
...
# 5. VISUALIZATIONS
# ==========================================
print("\n[4] GENERATING CAUSAL VALIDATION PLOTS...")

# Extract the interaction term name dynamically
int_term = [p for p in mod_did.params.index if 'EVM_Exposure' in p and 'Agency_Centered' in p][0]

# --- PLOT A: Event Study / Pre-Trend Validation ---
...
x_pos = [0, 1]
labels = ['Pre-Trend (1996-1998)\n[Placebo]', 'Treatment (1996-1999)\n[Actual DiD]']
...
# --- PLOT B: Diagnostic Residual Plots (For BP and JB tests) ---
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# 1. Residuals vs Fitted (Checks Heteroskedasticity / Breusch-Pagan)
...
# 2. Q-Q Plot (Checks Normality / Jarque-Bera)
...
# 1. Balanced Panel Dataset (1996, 1998, 1999)
panel.to_csv(os.path.join(CSV_DIR, "Step4_Panel_Data.csv"), index=False)

# 2. Causal Models (Pre-Trend, DiD, ANCOVA)
