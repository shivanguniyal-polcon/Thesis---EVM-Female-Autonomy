...
# ==========================================
# 1. ELECTORAL PROJECTION (PC -> 1991 District)
# ==========================================
print("\n[1] Projecting 1999 Election Data to 1991 Districts...")
...
df_1999['pc_name_clean'] = df_1999['pc_name_clean'].replace({'MUMBAI SOUTH CENTRA': 'MUMBAI SOUTH CENTRAL', 'PONDICHERRY': 'PUDUCHERRY'})
...
cw['pc_name_clean'] = cw['Constituency Clean'].str.upper().replace({'MUMBAI SOUTH CENTRA': 'MUMBAI SOUTH CENTRAL', 'PONDICHERRY': 'PUDUCHERRY'})
...
dist_electoral = m.groupby(['pc91_state_id', 'pc91_district_id', 'state_clean']).agg({
    'alloc_electors_f': 'sum', 'alloc_voters_f': 'sum', 'alloc_treated_electors_f': 'sum'
}).reset_index()
...
# ==========================================
# 2. DEMOGRAPHIC CONTROLS (1991 Census)
# ==========================================
print("\n[2] Loading 1991 Demographic Controls...")
...
# ==========================================
# 3. ECONOMIC AGENCY (1998 Economic Census)
# ==========================================
print("\n[3] Loading 1998 Economic Census (Female Enterprise Density)...")
ec98 = pd.read_csv(os.path.join(BASE_DIR, "Data/Economic Census 1991 Data/ec98_aggregated_1991_districts.csv"))

# Calculate % of enterprises owned by women
...
# THESIS BEST PRACTICE: Apply Inverse Hyperbolic Sine (IHS) transformation
# Why? Economic data is heavily right-skewed and contains zeros. 
# IHS acts like a log transformation but retains zero values safely.
ec98['Agency_IHS'] = np.arcsinh(ec98['Fem_Enterprise_Pct'])

# ==========================================
# 4. MERGE & MEAN-CENTERING
# ==========================================
print("\n[4] Merging Datasets & Preparing Interaction Terms...")
...
final_df = pd.merge(final_df, td_merged[['pc91_state_id', 'pc91_district_id', 'Urban_Pct']], on=['pc91_state_id', 'pc91_district_id'], how='left')
final_df = pd.merge(final_df, ec98[['pc91_state_id', 'pc91_district_id', 'Fem_Enterprise_Pct', 'Agency_IHS']], on=['pc91_state_id', 'pc91_district_id'], how='left')

final_df = final_df.dropna()
print(f"✅ Final Analytical Sample: {len(final_df)} districts.")

# THESIS BEST PRACTICE: Mean-Center the Moderator
# Why? It eliminates multicollinearity between the main effect and the interaction term.
# It also makes the main EVM coefficient interpretable as the effect at the "Average" district.
...
res_table = summary_col([mod_main, mod_interact], stars=True, float_format='%0.3f',
                        ...
                        info_dict={'N':lambda x: "{0:d}".format(int(x.nobs)), 'R2':lambda x: "{0:.3f}".format(x.rsquared)})
print(res_table)

# ==========================================
# 6. MARGINAL EFFECTS CALCULATION
# ==========================================
print("\n[6] CALCULATING MARGINAL EFFECTS OF EVM ACROSS AGENCY DISTRIBUTION...")
# Extract coefficients and covariance matrix for standard error calculation
beta_evm = mod_interact.params['EVM_Exposure']
# Statsmodels names interactions with a colon
...
for p in [0.10, 0.25, 0.50, 0.75, 0.90]:
    ...
    # Marginal Effect = Beta_EVM + Beta_Interaction * Agency
...
# 7. VISUALIZATIONS
# ==========================================
print("\n[7] GENERATING HETEROGENEITY VISUALIZATIONS...")

# --- PLOT A: Continuous Marginal Effects Plot ---
...
# --- PLOT B: Subsample Heterogeneity (High vs. Low Agency) ---
